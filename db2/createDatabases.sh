...
if [[ "${result}" == 0 ]]; then
    ...
    su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/homepage/db2/createDb.sql >/dev/null"; result="${?}"
    checkStatusDb "${result}" "E Unable to create database: Homepage. Exiting"
    su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/homepage/db2/appGrants.sql >/dev/null"; result="${?}" 
    checkStatusDb "${result}" "E Unable to grant rights on database: Homepage. Exiting." 
    su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/homepage/db2/initData.sql >/dev/null"; result="${?}"
    checkStatusDb "${result}" "E Unable to initialize data for database: Homepage. Exiting."
    su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/homepage/db2/reorg.sql >/dev/null"; result="${?}"
    checkStatusDb "${result}" "E Unable to run reorg on database: Homepage. Exiting."
    su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/homepage/db2/updateStats.sql >/dev/null"; result="${?}"
...
if [[ "${result}" == 0 ]]; then
    ...
    su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/files/db2/createDb.sql >/dev/null"; result="${?}"
    checkStatusDb "${result}" "E Unable to create database: Files. Exiting." 
    su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/files/db2/appGrants.sql >/dev/null"; result="${?}" 
...
if [[ "${result}" == 0 ]]; then
    ...
    su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/pushnotification/db2/createDb.sql >/dev/null"; result="${?}"
    checkStatusDb "${result}" "E Unable to create database: Push Notification. Exiting." 
    su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/pushnotification/db2/appGrants.sql >/dev/null"; result="${?}" 
...
if [[ "${result}" == 0 ]]; then
   ...
   su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/activities/db2/createDb.sql >/dev/null"; result="${?}"
   checkStatusDb "${result}" "E Unable to create database: Activities. Exiting." 
   su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/activities/db2/appGrants.sql >/dev/null"; result="${?}" 
...
if [[ "${result}" == 0 ]]; then
   ...
   su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/blogs/db2/createDb.sql >/dev/null"; result="${?}"
   checkStatusDb "${result}" "E Unable to create database: Blogs. Exiting." 
   su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/blogs/db2/appGrants.sql >/dev/null"; result="${?}" 
...
if [[ "${result}" == 0 ]]; then
   ...
   su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/dogear/db2/createDb.sql >/dev/null"; result="${?}"
   checkStatusDb "${result}" "E Unable to create database: Bookmarks. Exiting." 
   su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/dogear/db2/appGrants.sql >/dev/null"; result="${?}" 
...
if [[ "${result}" == 0 ]]; then
   ...
   su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/communities/db2/createDb.sql >/dev/null"; result="${?}"
   checkStatusDb "${result}" "E Unable to create database: Communities. Exiting." 
   su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/communities/db2/appGrants.sql >/dev/null"; result="${?}" 
   checkStatusDb "${result}" "E Unable to grant rights on database: Communities. Exiting." 
   su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/communities/db2/calendar-createDb.sql >/dev/null"; result="${?}"
   checkStatusDb "${result}" "E Unable to create table: Calendar. Exiting."
   su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/communities/db2/calendar-appGrants.sql >/dev/null"; result="${?}"
...
if [[ "${result}" == 0 ]]; then
   ...
   su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/forum/db2/createDb.sql >/dev/null"; result="${?}"
   checkStatusDb "${result}" "E Unable to create database: Forum. Exiting." 
   su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/forum/db2/appGrants.sql >/dev/null"; result="${?}" 
...
if [[ "${result}" == 0 ]]; then
   ...
   su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/metrics/db2/createDb.sql >/dev/null"; result="${?}"
   checkStatusDb "${result}" "E Unable to create database: Metrics. Exiting." 
   su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/metrics/db2/appGrants.sql >/dev/null"; result="${?}" 
...
if [[ "${result}" == 0 ]]; then
   ...
   su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/mobile/db2/createDb.sql >/dev/null"; result="${?}"
   checkStatusDb "${result}" "E Unable to create database: Mobile. Exiting." 
   su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/mobile/db2/appGrants.sql >/dev/null"; result="${?}" 
...
if [[ "${result}" == 0 ]]; then
   ...
   su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/profiles/db2/createDb.sql >/dev/null"; result="${?}"
   checkStatusDb "${result}" "E Unable to create database: Profiles. Exiting." 
   su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/profiles/db2/appGrants.sql >/dev/null"; result="${?}" 
...
if [[ "${result}" == 0 ]]; then
   ...
   su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/wikis/db2/createDb.sql >/dev/null"; result="${?}"
   checkStatusDb "${result}" "E Unable to create database: Wikis. Exiting." 
   su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/wikis/db2/appGrants.sql >/dev/null"; result="${?}" 
...
if [[ "${result}" == 0 ]]; then
   ...
   su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/library.gcd/db2/createDb.sql >/dev/null"; result="${?}"
   checkStatusDb "${result}" "E Unable to create database: GCD. Exiting." 
   su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/library.gcd/db2/appGrants.sql >/dev/null"; result="${?}" 
...
if [[ "${result}" == 0 ]]; then
   ...
   su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/library.os/db2/createDb.sql >/dev/null"; result="${?}"
   checkStatusDb "${result}" "E Unable to create database: OS. Exiting." 
   su - "${db2InstanceUser}" -c "db2 -td@ -sf ${icDbScriptDir}/library.os/db2/appGrants.sql >/dev/null"; result="${?}" 
