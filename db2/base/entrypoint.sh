...
# If the SETUP_URL variable has not been set, use the default URL
if [[ -z "${SETUP_URL}" ]]; then
    export SETUP_URL="https://raw.githubusercontent.com/chscott/ConnectionsDocker/master"
fi

# Download the run script and utilities
curl -L -O -J -s -S -f "${SETUP_URL}/db2/base/run.sh" || printf "W: Unable to download ${SETUP_URL}/db2/base/run.sh. Trying local run.sh...\n"
